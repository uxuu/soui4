...
if exist .\config\config.h del .\config\config.h
if %dynamicsoui%==1 (
	rem 全DLL模式
	SET configStr=#define DLL_SOUI_COM	//SOUI组件编译为dll
	echo !configStr!>>.\config\config.h
	SET configStr=#define DLL_CORE		//SOUI 内核编译为dll
	echo !configStr!>>.\config\config.h
) else if %dynamicsoui%==2 (
	rem 全LIB模式
	SET configStr=#define LIB_SOUI_COM		//SOUI组件编译为lib
	echo !configStr!>>.\config\config.h
	SET configStr=#define LIB_CORE		//SOUI 内核编译为lib
	echo !configStr!>>.\config\config.h
) else (
	rem 3=内核LIB,组件DLL(不能使用LUA脚本模块)]:
	SET configStr=#define DLL_SOUI_COM	//SOUI组件编译为dll
	echo !configStr!>>.\config\config.h
	SET configStr=#define LIB_CORE		//SOUI 内核编译为lib
	echo !configStr!>>.\config\config.h
